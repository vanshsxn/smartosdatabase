...
MV CloudCore is a full-stack academic project that combines **Operating Systems, DBMS, and Computer Networks** into one working platform.

The basic idea is simple: a user can submit computational jobs from a web dashboard, and those jobs are passed through an API to a separate **C++ task execution engine**. The engine handles scheduling, memory allocation, threads, resources, and job states, while PostgreSQL keeps track of the persistent data.

This project is meant to demonstrate how concepts that are usually taught separately in college can work together in one system.

> **Note:** MV CloudCore is not a real hypervisor and it is not intended to replace an actual operating system. The C++ component is an OS-inspired task execution and resource-management engine built for academic purposes.

---

## What does MV CloudCore actually do?
...
The interesting part of the project is that the scheduling and resource-management logic is not just simulated in the frontend.
...
The frontend mainly acts as the interface for controlling and monitoring the system.
...
This makes it easier to see how memory allocation changes when jobs start and finish.
...
MV CloudCore supports multiple tenants.
...
Keeping these parts separate makes it easier to work on the UI, API, C++ engine and database independently.
...
One of the main goals of MV CloudCore is to connect multiple subjects into one project.
...
I wanted to bring those concepts together into something that feels more like an actual application.

MV CloudCore is my attempt at building a small cloud-style task execution platform where the frontend, backend, database and C++ engine all have a clear role.

It is still an academic project rather than a production cloud platform, but the goal is to keep the core concepts real instead of hiding everything behind frontend simulations.
